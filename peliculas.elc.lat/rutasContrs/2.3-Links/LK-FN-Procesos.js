...
module.exports = {
	...
	obtieneLinksConEdicion: async (entidad, prodId, usuario_id) => {
		...
		let links = await baseDatos.obtieneTodosPorCondicion("links", {[campo_id]: prodId}, include);
	...
	datosLink: async (datos) => {
		// Datos del producto
		const regProd = await baseDatos.obtienePorId(datos.prodEnt, datos.prodId);
		datos.prodAprob = activos_ids.includes(regProd.statusRegistro_id); // antes era 'aprobados_ids'

		// campo_id
		const campo_id = comp.obtieneDesdeEntidad.campo_id(datos.prodEnt);
		datos[campo_id] = regProd.id;

		// grupoCol_id
		if (datos.prodEnt == "colecciones") datos.grupoCol_id = regProd.id;
		if (datos.prodEnt == "capitulos") datos.grupoCol_id = regProd.coleccion_id;
	...
	condicion: (link, usuario_id, tema) => {
		...
		cond.creadoAprobs = cond.creado || cond.creadoAprob || cond.aprobado;
	...
	sigProdInactivo: async ({producto, entidad, usuario_id}) => {
		...
		const link = sigProd
			? "/".concat(entidad, "/inactivar-captura") +
			  ...
			  "&prodEnt=".concat(sigProd.entidad) +
