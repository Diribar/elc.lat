...
module.exports = {
	...
	links: async (datos) => {
		...
		if (campos.includes("url")) {
			errores.url =
				(!datos.url && inputVacio) ||
				(datos.url && comp.validacs.longitud(datos.url, 5, 120)) ||
				(!datos.url.includes("/") && "Por favor ingresá una url válida") ||
				...
				(provsListaNegra.some((n) => datos.url.includes(n)) && "Los videos de ese portal son ajenos a nuestro perfil") ||
				"";
			if (!errores.url) {
...
const validaLinkRepetidos = async (datos) => {
	...
	// Obtiene casos
	const id = await validaFM.repetidos(["url"], datos);
	if (id) {
		...
		datos = {entidad: prodEnt, id: prodId, entidadNombre: "link"};
