...
module.exports = {
	...
	datosDuros: async function (campos, datos) {
		...
		if (datos.nombreOriginal && !errores.nombreOriginal && datos.anoEstreno && !errores.anoEstreno && datos.entidad) {
			...
			if (id) errores.nombreOriginal = comp.cartelRepetido({...datos, id});
		}
		...
		if (datos.nombreCastellano && !errores.nombreCastellano && datos.anoEstreno && !errores.anoEstreno && datos.entidad) {
			...
			if (id) errores.nombreCastellano = comp.cartelRepetido({...datos, id});
		}
	...
	datosAdics: (campos, datos) => {
		// Definir variables
		let errores = {};
		let camposPosibles = ["religion_id", "bhr", "tipoActuacion_id"];
		// Datos generales
		for (let campo of camposPosibles)
		...
		// Rclv
		const rclvCampos_id = [...variables.entidades.rclvCampos_id, "sinRclv"];
		if (campos.some((n) => rclvCampos_id.includes(n)))
			errores.rclv = rclvCampos_id.every((n) => !datos[n] || datos[n] == ninguno_id) ? rclvSinElegir : "";
	...
	repetidos: async (campos, datos) => {
		...
		// Averigua si existe
		const existe = await baseDatos.obtienePorCondicion(datos.entidad, condicion);
