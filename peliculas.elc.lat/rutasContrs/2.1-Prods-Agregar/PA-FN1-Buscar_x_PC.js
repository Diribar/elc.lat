...
module.exports = {
	...
	reemplPeliPorColec: async (productos) => {
		...
		let reemplazaSiCorresponde = async () => {
			...
			for (let producto of productos) {
				...
				const detalles = producto.entidad == "peliculas" && APIsTMDB.details("movie", producto.TMDB_id);
				posiblesColecs.push(detalles);

				// Cada cuatro películas, hace una pausa - es necesario para que no se trabe la API
			...
			posiblesColecs.forEach((posibleColec, indice) => {
				// Si no es una película, interrumpe
				if (!posibleColec) return;

				// Si es una película, agrega el director
				if (posibleColec.credits && posibleColec.credits.crew) {
					const direcArray = posibleColec.credits.crew.filter((n) => n.department == "Directing");
					let direccion = procsComp.valores(direcArray);
					if (direccion.includes(", ")) direccion = direccion.split(", ").slice(0, 2).join(", ");
					productos[indice].direccion = direccion;
				}

				// Si no pertenece a una colección, interrumpe
				if (!posibleColec.belongs_to_collection) return;
	...
	organizaLaInfo: async ({productos, hayMas}) => {
		...
		let agregaInfoDetalleParaColeccionesDesdeAPI = async () => {
			...
			colecciones.forEach((coleccion, indice) => {
				...
				// Acciones si es una serie de TV
					...
					const cantTemps = coleccion.seasons.filter((n) => n.season_number).length; // numTemp mayor a cero
		...
		let agregaInfoDesdeBD = async () => {
			...
			let peliculas = productos.find((n) => n.entidad == "peliculas") ? baseDatos.obtieneTodos("peliculas") : [];
			let colecciones = productos.find((n) => n.entidad == "colecciones")
				? baseDatos.obtieneTodos("colecciones", "capitulos")
				: [];
			...
			for (let producto of productos) {
				...
				if (prodBD) {
					...
					producto.nombreCastellano = prodBD.nombreCastellano;
					producto.nombreOriginal = prodBD.nombreOriginal;
					producto.avatar = prodBD.avatar;
					producto.anoEstreno = prodBD.anoEstreno;
...
const FN = {
	...
	agregaQuitaCapsCollection: async (coleccion) => {
		// Variables
		const capitulosELC = await baseDatos.obtieneTodosPorCondicion("capitulos", {coleccion_id: coleccion.id});
		...
		for (let capituloELC of capitulosELC)
			if (!coleccion.TMDB_ids_vTMDB.includes(capituloELC.TMDB_id) && capituloELC.statusRegistro_id == inactivo_id) {
				...
				baseDatos.eliminaPorId("capitulos", capituloELC.id);
			}
	...
	agregaCapsTV: async (coleccion) => {
		...
		coleccion = {...coleccion, ...(await baseDatos.obtienePorId("colecciones", coleccion.id))};
		...
		for (let numTemp = 1; numTemp <= coleccion.cantTemps; numTemp++) {
			...
			for (let TMDB_id_vTMDB of coleccion.TMDB_ids_vTMDB)
				if (!coleccion.TMDB_ids_vELC.includes(TMDB_id_vTMDB)) {
					...
					// Guarda el registro
					await baseDatos.agregaRegistroIdCorrel("capitulos", datosCap);
				}
		}
