...
module.exports = {
	obtieneInfoDeMovie: async function (datos) {
		...
		// Obtiene las API
		const detalles = APIsTMDB.details("movie", datos.TMDB_id);
		const creditos = APIsTMDB.credits("movie", datos.TMDB_id);
		const datosAPI = await Promise.all([detalles, creditos]).then(([a, b]) => ({...a, ...b}));
	...
	datosPelis: function (datosAPI) {
		...
		if (!idiomas.find((n) => n.id == datos.idiomaOriginal_id)) datos.idiomaOriginal_id = null;
		if (datosAPI.production_countries.length > 0) {
			// Le agrega el o los países
			datos.paises_id = datosAPI.production_countries.map((n) => n.iso_3166_1).join(" ");

			// Si no hay idioma y es un sólo país, le pone el idioma de ese país
