...
module.exports = {
	...
	RCLIC_personajes: (datos) => {
		...
		const respuesta = !datos.religion_id
			? "Necesitamos saber su religión"
			: datos.religion_id == "CFC"
			? !datos.genero_id
				...
				: ""
			: !["MP", "VPC"].includes(datos.religion_id)
	...
	leyenda: (datos) => {
		...
		variables.camposRevisar.rclvs
			.filter((n) => n[entidad] && ["leyNombre", "hoyEstamos_id"].includes(n.nombre))
	...
	// Épocas del año
	carpetaAvatars: (datos) => (!datos.carpetaAvatars ? selectVacio : ""),
};
...
const nombreApodo = async ({datos, campo}) => {
	...
	if (!mensaje && dato) {
		...
		// Nombre repetido
		if (!mensaje) {
			let id = await validaFM.repetidos([campo], datos);
