...
const layoutDefault_id = 13; // El 'default' es "Santoral + Azar"
...
module.exports = {
	obtiene: {
		...
		variables: async (req, res) => {
			...
			const datos = {
				...
				...{rclvsNombre: variables.entidades.rclvsNombre, religiones},
				...{filtrosConsConDefault, epocasEstreno, unDia, setTimeOutStd},
			...
			if (req.session.usuario && req.session.usuario.id) {
				...
				datos.usuarioTienePPP = await baseDatos
		...
		cabecera: async (req, res) => {
			...
			const cabecera_id =
				req.query.id ||
				(req.session.configCons && req.session.configCons.id) ||
				(req.session.usuario && req.session.usuario.configCons_id) ||
				cnCabeceraDefault.id;
			let cabecera =
				["string", "number"].includes(typeof cabecera_id) &&
				(await baseDatos.obtienePorId("consRegsCabecera", cabecera_id));

			// Errores de cabecera
			if (
				!cabecera || // no se encontró una cabecera
				(!usuario_id && cabecera.usuario_id != 1) || // usuario no logueado y usuario de cabecera no es default
				(usuario_id && ![usuario_id, 1].includes(cabecera.usuario_id)) // el usuario_id no es suyo ni el default
			)
				cabecera = cnCabeceraDefault;
		...
		preferencias: async (req, res) => {
			...
			// De lo contrario, toma sus datos
				...
				const prefsSession = req.session.configCons || null;
			...
			if (
				!preferencias ||
				!preferencias.layout_id ||
				!cnLayouts_ids.includes(Number(preferencias.layout_id)) // no está entre los layouts posibles
			)
				preferencias = {layout_id: layoutDefault_id};
			else {
				// Miscelaneas
	...
	cambiosEnBD: {
		...
		creaConfig: async (req, res) => {
			...
			const {id} = await baseDatos.agregaRegistro("consRegsCabecera", objeto);
		...
		guardaConfig: async (req, res) => {
			...
			// Crea la variable que contiene todas las preferencias válidas
			const filtrosCons = Object.keys(variables.filtrosCons);
			filtrosCons.push("layout_id", "excluyeBC", "religiones");

			// Quita algunas preferencias
			for (let prop in prefs)
				if (
					prefs[prop] == filtrosConsConDefault[prop] || // el valor es un default
					!filtrosCons.includes(prop) // la preferencia no pertenece a los filtros
				)
					delete prefs[prop];

			// Acciones para edición
			if (cabecera.edicion) baseDatos.actualizaPorId("consRegsCabecera", id, {nombre});
			// Acciones para 'nuevo' y 'actualizar campos'
				...
				if (!cabecera.nuevo) await baseDatos.eliminaPorCondicion("consRegsPrefs", {cabecera_id: id});

				// Guarda la nueva información
				for (let prop in prefs) {
					const valor = Array.isArray(prefs[prop]) ? prefs[prop].join(",") : prefs[prop];
					const objeto = {cabecera_id: id, campo: prop, valor};
					baseDatos.agregaRegistro("consRegsPrefs", objeto);
				}
			}
		...
		eliminaConfig: async (req, res) => {
			...
			await baseDatos.eliminaPorCondicion("consRegsPrefs", {cabecera_id});

			// Se elimina el registro de cabecera de la configuración
			await baseDatos.eliminaPorId("consRegsCabecera", cabecera_id);
	...
	resultados: async (req, res) => {
		...
		let pppRegistros = usuario_id ? baseDatos.obtieneTodosPorCondicion("pppRegistros", {usuario_id}, "detalle") : [];
