...
const procsPR = require("../2.1-Prods-RUD/PR-FN-Procesos");
...
module.exports = {
	...
	tablRev: {
		obtieneProdsRclvs: () => {
			...
			for (let opcion of ["ST", "SH", "IN", "RC"]) respuesta[opcion] = statusErrores.filter((n) => n[opcion]);
		...
		obtieneProds1: async (revId) => {
			// Variables
			let include = [...variables.entidades.prodAsocs, ...variables.entidades.rclvAsocs];
			let productos = [];

			// Obtiene todas las ediciones
			let ediciones = await baseDatos.obtieneTodos("prodEdics", include);

			// Elimina las ediciones con rclv no aprobado
			ediciones = ediciones.filter(
				(edicion) =>
					!variables.entidades.rclvAsocs.some((rclv) => edicion[rclv] && edicion[rclv].statusRegistro_id != aprobado_id)
			);

			// Obtiene los productos
			ediciones.map((n) => {
				const entidad = comp.obtieneDesdeCampo_id.prodEnt(n);
				const asociacion = comp.obtieneDesdeEntidad.asociacion(entidad);
				productos.push({
					...n[asociacion],
					entidad,
					fechaRef: n.editadoEn,
					fechaRefTexto: comp.fechaHora.diaMesUTC(n.editadoEn),
					edicId: n.id,
				});
			});
		...
		obtieneProds3: async () => {
			...
			let registros = await Promise.all([
				baseDatos.obtieneTodosPorCondicion("peliculas", {statusRegistro_id}),
				baseDatos.obtieneTodosPorCondicion("capitulos", {statusRegistro_id}),
			])
				.then((n) => [
		...
		obtieneRclvs1: async (revId) => {
			...
			// AL: Altas
			campos = {...camposFijos, status_id: creado_id, include: [...variables.entidades.prods, "prodEdics"]};
		...
		obtieneRclvs2: async (revId) => {
			// Variables
			let include = variables.entidades.rclvAsocs;
			let rclvs = [];

			// Obtiene todas las ediciones ajenas
			let ediciones = await baseDatos.obtieneTodos("rclvEdics", include);

			// Obtiene los rclvs originales y deja solamente los rclvs aprobados
			if (ediciones.length) {
				// Obtiene los rclvs originales
				ediciones.map((n) => {
					let entidad = comp.obtieneDesdeCampo_id.rclvEnt(n);
		...
		procesaProdsRclvs: (registros) => {
			...
			for (let rubro in registros)
				registros[rubro] = registros[rubro].map((n) => {
					...
					const fechaRefTexto = comp.fechaHora.diaMesUTC(fechaRef);
		...
		procesaProds: (productos) => {
			...
			for (let rubro in productos)
				productos[rubro] = productos[rubro].map((n) => {
					...
					const fechaRef = n.fechaRef || n.altaRevisadaEn || n.statusSugeridoEn;
					const fechaRefTexto = n.fechaRefTexto || comp.fechaHora.diaMesUTC(fechaRef);
					const abrev = entidad.slice(0, 3).toUpperCase();
		...
		procesaRclvs: (rclvs) => {
			...
			for (let rubro in rclvs)
				rclvs[rubro] = rclvs[rubro].map((n) => {
					...
					const fechaRefTexto = n.fechaRefTexto ? n.fechaRefTexto : comp.fechaHora.diaMesUTC(fechaRef);
					const abrev = entidad.slice(0, 3).toUpperCase();
	...
	cambioStatus: {
		obtieneDatos: async (req) => {
			...
			// Más variables
			const idOrigen = "/?id=" + id + "&origen=" + (origen || "TE");
			const revId = req.session.usuario.id;
			const ahora = comp.fechaHora.ahora();
			const revisorPERL = req.session.usuario && req.session.usuario.rol.revisorPERL;
			const petitFams = comp.obtieneDesdeEntidad.petitFams(entidad);
			const usuario_id = original.statusSugeridoPor_id;
			const campoDecision = petitFams + (aprobado ? "Aprob" : "Rech");

			// Fin
			return {
				...{entidad, id, original, statusOriginal_id, statusFinal_id},
				...{codigo, producto, rclv, motivo_id, comentario, aprobado},
				...{idOrigen, revId, ahora, revisorPERL, petitFams, tema, usuario_id, campoDecision},
			};
		},
		actualizaProdAprobEnLink: async (coleccion_id, statusCol) => {
			...
			const capsID = await baseDatos.obtieneTodosPorCondicion("capitulos", {coleccion_id}).then((n) => n.map((m) => m.id));

			// Actualiza el campo 'prodAprob' a los links de la colección
			await baseDatos.actualizaPorCondicion("links", {capitulo_id: capsID}, {prodAprob});

			// Fin
			return;
		},
		avatarRclv: (reqFile, original) => {
			// Si se recibió un archivo, lo mueve de 'Provisorio' a 'Final' y elimina el original
			if (reqFile) {
				comp.gestionArchivos.mueveImagen(reqFile.filename, carpProvisorio, carpRclvs.final);
				if (original.avatar) comp.gestionArchivos.elimina(carpRclvs.revisar, original.avatar);
			}

			// Si proviene del status 'creado', mueve el avatar de 'Revisar' a 'Final'
			else if (original.statusRegistro_id == creado_id && original.avatar)
				comp.gestionArchivos.mueveImagen(original.avatar, carpRclvs.revisar, carpRclvs.final);
		...
		prodsAsocs: async (entidad, id) => {
			...
			for (let prodEnt of prodEnts) {
				...
				FN.actualizaLosProdsVinculadosNoAprobados({entidad: prodEnt, campo_id, id});

				// Obtiene los productos aprobados vinculados
				const condicion = {[campo_id]: id, statusRegistro_id: aprobado_id};
				let prodsVinculados = await baseDatos.obtieneTodosPorCondicion(prodEnt, condicion);
				...
				for (let prodVinculado of prodsVinculados) {
					...
					const errores = await validaFM.consolidado({datos: prodVinculado});
					...
					baseDatos.actualizaPorId(prodEnt, prodVinculado.id, objeto);

					// Si es una colección en status creadoAprob_id, actualiza sus capítulos que tengan status aprobado
					if (prodEnt == "colecciones" && errores.impideAprobado) {
						const condicion = {coleccion_id: prodVinculado.id, statusRegistro_id: aprobado_id};
						baseDatos.actualizaPorCondicion("capitulos", condicion, statusCreadoAprob);
					}
				}
			}
		},
		edicRclvAprobRech: async (entidad, original, revId) => {
			// Variables
			...
			const rclvActual = await baseDatos.obtienePorId(entidad, original.id, include);
			...
			for (let campoRevisar of camposRevisar) {
				...
				// Valores a comparar
				const {valorAprob, valorDesc} = FN.valoresComparar(original, rclvActual, relacInclude, campo);
				...
				baseDatos.agregaRegistro("edicsHistorial", datosCompleto);
			...
			const campoEdic =
				ediciones.edicsAprob > ediciones.edicsRech
					...
					? "edicsRech"
					: "";
			if (campoEdic) baseDatos.variaElValorDeUnCampo("usuarios", usuario_id, campoEdic);
	...
	edicForm: {
		// Vista
		reemplAvatarAutom: async function ({entidad, original, edicion}) {
			// Averigua si se debe reemplazar el avatar en forma automática
			const reemplAvatarAutom =
			...
			if (reemplAvatarAutom) {
				// Mueve el archivo avatar a la carpeta final
				await FN.procsParticsAvatar({entidad, original, edicion, aprob: true});
				await baseDatos.actualizaPorId(entidad, original.id, {avatar: edicion.avatar}); // REGISTRO ORIGINAL: actualiza el campo 'avatar' en el registro original
				await baseDatos.actualizaPorId("prodEdics", edicion.id, {avatar: null, avatarUrl: null}); // REGISTRO EDICION: borra los campos de 'avatar' en el registro de edicion
			}

			// Fin
			return reemplAvatarAutom;
		},
		reemplAvatarManual: ({codigo, titulo, original, edicion}) => {
			// Variables - a partir de original y edición
			const avatar = procsFM.obtieneAvatar(original, edicion);
			const nombre = original.nombreCastellano || original.nombre;
			...
			const respuesta = {
				...{codigo, titulo},
				...{registro: original, prodOrig: original, prodEdic: edicion},
				...{motivos, avatar, avatarsExternosPelis},
	...
	// Links - Vista, API
	obtieneSigProd: async (datos) => {
		// Variables
		const {ediciones: edicsPend, prods} = cantLinksVencPorSem["0"];
		let respuesta;

		// Si no hay nada para revisar, interrumpe la función
		if (!prods) return;

		// Ediciones
		if (edicsPend) {
			const ediciones = await FN_links.obtieneLinks.ediciones();
			respuesta = await FN_links.obtieneProdLink({links: ediciones, datos});
			if (respuesta) return respuesta;
		}

		// Obtiene los links
		const links = await FN_links.obtieneLinks.links(); // obtiene los links 'a revisar'
		if (!links.length) return; // Si no hay links, interrumpe la función

		// Inactivar
		const inactivar = links.filter((n) => n.statusRegistro_id == inactivar_id);
		respuesta = await FN_links.obtieneProdLink({links: inactivar, datos});
		if (respuesta) return respuesta;

		// Altas
		const creados = links.filter((n) => n.statusRegistro_id == creado_id);
		respuesta = await FN_links.obtieneProdLink({links: creados, datos});
		if (respuesta) return respuesta;

		// Estándar - Capítulos
		const capitulos = links.filter((n) => n.capitulo_id);
		respuesta = await FN_links.obtieneProdLink({links: capitulos, datos});
		if (respuesta) return respuesta;

		// Estándar - Películas y Colecciones
		const pelisColes = links.filter((n) => !n.capitulo_id);
		respuesta = await FN_links.obtieneProdLink({links: pelisColes, datos});
		if (respuesta) return respuesta;
	...
	// Links - Vista
	prodSinLinks: (producto, urlAnterior) => {
		// Variables
		let informacion;
	...
	descargaAvatarOriginal: async (original, entidad) => {
		...
		const ruta = familias == "productos" ? carpProds.final : carpRclvs.final;
		const nombre = Date.now() + path.extname(original.avatar);
		const rutaNombre = path.join(ruta, nombre);
		comp.gestionArchivos.descarga(original.avatar, rutaNombre);

		// Actualiza el registro 'original'
		await baseDatos.actualizaPorId(entidad, original.id, {avatar});
	...
	edicAprobRech: async function (objeto) {
		...
		if (aprob) {
			...
			await baseDatos.actualizaPorId(entidad, original.id, datos);

			// 3. Si es una colección, revisa si corresponde actualizar ese campo en sus capítulos
			if (entidad == "colecciones") await procsPR.transfDatosDeColParaCaps(original, edicion, campo);
		}
		...
		if (fueProvistoPorElUsuario1 || fueProvistoPorElUsuario2) {
			// Actualiza la tabla de 'edicsHistorial'
			...
			// Agrega el registro
			baseDatos.agregaRegistro("edicsHistorial", datosEdic);

			// Aumenta el campo 'edicsAprob/edicsRech' en el registro del usuario
			baseDatos.variaElValorDeUnCampo("usuarios", edicion.editadoPor_id, decision);
		...
		await baseDatos.actualizaPorId(entEdic, edicion.id, {[campo]: null});
	...
	procsParticsAvatar: ({entidad, original, edicion, aprob}) => FN.procsParticsAvatar({entidad, original, edicion, aprob}),
};
...
const FN_links = {
	obtieneLinks: {
		ediciones: async () => {
			const include = variables.entidades.prodAsocs;
			const ediciones = await baseDatos.obtieneTodos("linkEdics", include);
			return ediciones;
		},
		links: async () => {
			// Variables
			...
			const originales = await baseDatos
	...
	puleLosResultados: async ({productos, revId}) => {
		...
		if (productos.length > 1) {
			...
			productos
				...
				.sort((a, b) => (a.numCap && b.numCap ? a.numCap - b.numCap : a.numCap ? -1 : 0)) // por capítulo
				.sort((a, b) => (a.numTemp && b.numTemp ? a.numTemp - b.numTemp : a.numTemp ? -1 : 0))
	...
	fechaVencim: ({link, ahora, IN}) => {
		...
		const cantLinksVencPorSem_valores = Object.values(cantLinksVencPorSem)
		...
		const techo = cantLinksVencPorSem.promSem[entidad];
		const semana = cantLinksVencPorSem_valores.findIndex((n) => n < techo) + linkSemInicial;

		// Actualiza provisoriamente los links vencidos por semana
		cantLinksVencPorSem[semana][entidad]++;
...
const FN_edicion = {
	valoresParaMostrar: async (registro, relacInclude, campoRevisar, esEdicion) => {
		...
		const casosEspeciales = [
			...["bhr", "musical", "color", "deporte", "crueldad", "capEnCons"], // productos
		...
		else if (!esEdicion && variables.entidades.rclvCampos_id.includes(campo) && registro[campo] == ninguno_id)
			resultado = null;
	...
	actualizaArchivoAvatar: async ({entidad, original, edicion, aprob}) => {
		...
		const familias = comp.obtieneDesdeEntidad.familias(entidad);
		const rutaFinal = familias == "productos" ? carpProds.final : carpRclvs.final;
		const rutaRevisar = familias == "productos" ? carpProds.revisar : carpRclvs.revisar;

		// Reemplazo
		if (aprob) {
			// ARCHIVO ORIGINAL: si el 'avatar original' es un archivo, lo elimina
			if (avatarOrig && !url) {
				const rutaNombre = path.join(rutaFinal, avatarOrig);
				if (comp.gestionArchivos.existe(rutaNombre)) comp.gestionArchivos.elimina(rutaFinal, avatarOrig);
			}

			// ARCHIVO NUEVO: mueve el archivo de edición a la carpeta definitiva
			comp.gestionArchivos.mueveImagen(avatarEdic, rutaRevisar, rutaFinal);
		}

		// Rechazo - Elimina el archivo de edicion
		else if (!aprob) comp.gestionArchivos.elimina(rutaRevisar, avatarEdic);
...
const FN = {
	...
	actualizaLosProdsVinculadosNoAprobados: async ({entidad, campo_id, id}) => {
		...
		// Actualiza
		await baseDatos.actualizaPorCondicion(entidad, condicion, datos);
	...
	statusFinalMasMotivo: async ({codigo, desaprueba, rclv, entidad, original, req}) => {
		...
		const impideAprobado = statusAprob && !rclv ? await validaFM.consolidado({datos}).then((n) => n.impideAprobado) : null;
	...
	procsParticsAvatar: async ({entidad, original, edicion, aprob}) => {
		...
		if (familias == "productos") {
			...
			if (!aprob && url && url.includes("/") && entidad != "capitulos") {
				...
				// Descarga el url
				const rutaNombre = path.join(carpProds.final, original.avatar);
				await comp.gestionArchivos.descarga(url, rutaNombre);
			}

			// 2. Borra el campo 'avatarUrl' en el registro de edicion
			await baseDatos.actualizaPorId("prodEdics", edicion.id, {avatarUrl: null});
		}
		...
		if (entidad == "epocasDelAno" && aprob) {
			const rutaNombreOrigen = path.join(carpRclvs.final, edicion.avatar);
			const carpAvatar = edicion.carpetaAvatars || original.carpetaAvatars;
			const rutaNombreDestino = path.join(carpImgsComp, "3-EpocasDelAno", carpAvatar, edicion.avatar);
