...
module.exports = async (req, res, next) => {
	...
	v = {
		...
		vistaAnteriorTablero: v.usuario.rol.autTablEnts ? [v.vistaSinCaptura, v.vistaTablero] : [v.vistaSinCaptura],
	};

	// Más variables
	if (entidad != "usuarios") v.include.push("ediciones");
	if (entidad == "capitulos") v.include.push("coleccion");
	v.registro = await baseDatos.obtienePorId(entidad, id, v.include);
		...
		if (v.creadoEn > v.haceUnaHora) {
			...
			informacionm = {
				mensajes: [
					...
					"Estará disponible para su revisión el [horario]",
				],
				horario: comp.fechaHora.nuevoHorario(1, v.creadoEn),
	...
	if (captura) {
		informacion = {
			mensajes: [
				"Este registro está capturado por " + captura.capturadoPor.apodo,
				"Estará liberado a más tardar el [horario]",
			],
			horario: comp.fechaHora.nuevoHorario(1, captura.capturadoEn),
	...
	if (captura) {
		informacion = {
			mensajes: [
				"Esta captura terminó el [horario]",
	...
	if (captura) {
		...
		const prodRclvCapturado = await baseDatos.obtienePorId(capturaEnt, capturaEnt_id);

		// Acciones si no existe ese registro
		if (!prodRclvCapturado) {
			await baseDatos.eliminaPorCondicion("capturas", {entidad: capturaEnt, entidad_id: capturaEnt_id});
