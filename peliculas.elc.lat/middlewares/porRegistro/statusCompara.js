...
module.exports = async (req, res, next) => {
	...
	const statusDistinto = tema == "/unificar-status";
	const statusIgual = ["/cambiar-el-motivo", "/historial"].includes(tema);
	let informacion;
	...
	if (statusAlineado && statusDistinto) {
		...
		// Variables
		const {urlAnterior} = req.session;
		const urlStatus = "/revision/unificar-status/" + entidad + "/?id=" + id;
