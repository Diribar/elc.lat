...
module.exports = async (req, res, next) => {
	...
	if (!statusEsperado_ids && entidades.includes(baseUrl)) {
		statusEsperado_ids = ["edicion", "ordena-capitulos"].includes(tarea) // edición y reordenar capítulos
			? activos_ids
			: tarea == "inactivar"
			? aprobados_ids
	...
	if (!statusEsperado_ids && baseUrl == "revision") {
		statusEsperado_ids = ["alta", "rechazar"].includes(tarea) // revisar alta y rechazar
			? [creado_id]
			: ["edicion", "ordena-capitulos"].includes(tarea) // edición y reordenar capítulos
			? aprobados_ids
			...
			? activos_ids
			: ["cambiar-el-motivo", "unificar-status"].includes(tarea) // correcciones
			? [inactivo_id]
			: null;
	}
	...
	if (!statusEsperado_ids.includes(statusRegistro_id)) {
		...
		if (statusRegistro_id > aprobado_id) {
			const regHistorial = await baseDatos.obtienePorCondicion("statusHistorial", {entidad, entidad_id: id});
			if (!regHistorial) return next();
		}

		// Variables para el mensaje
		const statusActual = statusRegistros.find((n) => n.id == statusRegistro_id);
		const statusActualNombre = statusActual.nombre;

		// Variables para el ícono
		const origen = req.query.origen || (baseUrl == "revision" ? (familia ? "TE" : "TU") : "DT");
