...
module.exports = async (req, res, next) => {
	...
	if (!edicId) {
		...
		if (revision) {
			...
			edicion = await baseDatos.obtienePorCondicion(entEdic, {[campo_id]: id});
			...
			edicion = await baseDatos.obtienePorCondicion(entEdic, condicion);
		}
	...
	if (edicId) {
		...
		const edicion = await baseDatos.obtienePorId(entEdic, edicId);
