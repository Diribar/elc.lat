...
window.addEventListener("load", async () => {
	...
	for (let indice = 0; indice < v.casos; indice++) {
		...
		DOM.aprobar[indice].addEventListener("click", async () => {
			...
			const ruta = rutaEdicion + campo + "&aprob=true";
			resultado = await fetch(ruta).then((n) => n.json());

			// Fin
			FN.accionesSiSeTermino();
			return;
		});
		...
		DOM.rechazar[indice].addEventListener("click", async () => {
			...
			const ruta = rutaEdicion + campo + "&motivo_id=" + motivo_id;
			resultado = await fetch(ruta).then((n) => n.json());
...
// Rutas
const rutaEdicion = "/revision/api/re-edicion-aprob-rech/?entidad=" + entidad + "&id=" + id + "&edicId=" + edicId + "&campo=";
