...
window.addEventListener("load", async () => {
	...
	DOM.form.addEventListener("change", async (e) => {
		...
		if (["personaje_id", "hecho_id"].includes(nombre) && valor != 1) {
			const rclvEnt = nombre == "personaje_id" ? "personajes" : "hechos";
			const rclv = await fetch(rutas.obtieneRclv + rclvEnt + "&id=" + valor).then((n) => n.json());
...
const rutas = {
	...
	eliminaEdicN: "/" + entidad + "/api/pr-elimina-nueva",
	eliminaEdicG: "/" + entidad + "/api/pr-elimina-guardada/?id=" + id,
