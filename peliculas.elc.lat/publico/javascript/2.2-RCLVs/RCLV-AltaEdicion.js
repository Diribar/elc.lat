...
window.addEventListener("load", async () => {
	...
	if (personajes || hechos) {
		...
		DOM = {
			...
			religion_ids: document.querySelectorAll("form #sectorRCLIC input[name=religion_id]"),
			religionCar: document.querySelector("form #sectorRCLIC .input[name='religion_id'][value='CFC']"), // sólo para personajes
			soloCfc: document.querySelectorAll("form #sectorRCLIC input[name=soloCfc]"), // sólo para hechos
	...
	const FN = {
		impactos: {
			...
			genero: {
				consolidado: async function () {
					...
					if (personajes) {
						...
						// Impacto por la combinación de religion_id y género
						FN.impactos.carGenero();
					}
			...
			carGenero: () => {
				...
				if (DOM.religionCar.checked && v.genero_id) {
		...
		validacs: {
			...
			RCLIC: {
				personajes: async () => {
					// Variables
					const religion_id = FN_aux.opcElegida(DOM.religion_ids);
					let params = "RCLIC_personajes";
					params += "&religion_id=" + religion_id;
		...
		startUp: async function (forzar) {
			...
			// RCLIC
			if (
				(personajes && FN_aux.opcElegida(DOM.religion_ids)) ||
	...
	DOM.form.addEventListener("change", async (e) => {
		// Ediciones por desvíos del estándar
		edicionesChange(e);
		...
		if (["genero_id", "plural_id"].includes(campo)) {
			...
			if (personajes && OK.genero_id && FN_aux.opcElegida(DOM.religion_ids) == "CFC") await FN.validacs.RCLIC.personajes();
		}
		...
		if (v.camposEpoca.includes(campo)) {
			...
			if (OK.epocaOcurrencia) {
				if (personajes && FN_aux.opcElegida(DOM.religion_ids) == "CFC") await FN.validacs.RCLIC.personajes();
		...
		if (v.camposRCLIC && v.camposRCLIC.includes(campo)) {
			if (campo == "religion_id") FN.impactos.carGenero(); // son campos afectados por la combinación de religion y genero
...
const FN_aux = {
	opcElegida: (opciones) => {
		// Radio
		if (opciones[0].type == "radio") return Array.from(opciones).find((opcion) => opcion.checked)?.value || "";
		// Check
		else {
			let valor = "";
