...
const obtiene = {
	...
	cabecera: () => {
		// Es la cabecera de la configuración
...
const actualiza = {
	...
	statusInicialPrefs: async (texto) => {
		...
		for (const prefSimple of DOM.prefsSimples)
			prefSimple.value = preferencias[prefSimple.name] || v.filtrosConsConDefault[prefSimple.name] || "";
		...
		// Religiones
		for (const religion of DOM.religsInput)
			religion.checked = preferencias && preferencias.religiones && preferencias.religiones.includes(religion.value);
		if (!Array.from(DOM.religsInput).some((n) => n.checked)) DOM.religsInput[0].checked = true;
...
const excluyeBC = () => (DOM.excluyeInput.checked ? "Descarta" : "Incluye") + " películas con baja calificación";
